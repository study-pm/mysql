...
- До операции индексирования полей таблиц БД должна быть проделана работа по анализу наиболее частых запросов к БД. Только в этом случае использование индексов позволит оптимизировать работу БД.
...
При работе с `ALTER TABLE` добавление записей происходит при помощи команды **`ADD INDEX`**.

MySQL при помощи этой команды позволяет создавать индексы `PRIMARY KEY` (создать индекс такого типа при помощи `CREATE INDEX` нельзя).
